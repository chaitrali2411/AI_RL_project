...
class Environment:
    def __init__(self, grid_size=GRID_SIZE, initial_food=INITIAL_FOOD_COUNT):
        ...
        self.food_collected = 0  # Counter for collected food
        self.update_grid()
    ...
    def check_food(self):
        if tuple(self.mouse_position) in self.food_positions:
            ...
            self.food_collected += 1  # Increment food counter
            return FOOD_REWARD
...
def render(env, message=None):
    ...
    # Display food collected count and optional message
    font = pygame.font.SysFont(None, 24)
    status_text = font.render(f"Food Count = {env.food_collected}   {message if message else ''}", True, RED)
    screen.blit(status_text, (10, GRID_SIZE * CELL_SIZE + 40))
...
def train_and_visualize(env, model, episodes=100):
    for episode in range(episodes):
        ...
        # Display food count and "Mouse is died" message if mouse dies
        if env.mouse_energy <= 0:
            render(env, message="Mouse is died")
            pygame.time.delay(1000)
            print(f"Episode {episode+1} completed. Food Count = {env.food_collected}   Mouse is died")
